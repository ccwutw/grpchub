...
func newProxy(addr, cacert, certfile, keyfile string) (*httputil.ReverseProxy, error) {
	...
	caPool, err := message.NewCAPool(cacert)
	if err != nil {
		return nil, err
	}
...
// healthServer is used to implement message.HealthServer
type healthServer struct {
	message.UnimplementedHealthServer
}

// Status implements message.HealthServer Status request
func (s *healthServer) Status(context.Context, *message.StatusRequest) (*message.StatusReply, error) {
	log.Printf("Received StatusRequest")
	return &message.StatusReply{Code: http.StatusOK, Message: "OK"}, nil
}
...
func main() {
	...
	gs := grpc.NewServer()
	message.RegisterHealthServer(gs, &healthServer{})

	// Enable reflection for grpcurl
	reflection.Register(gs)

	mux := http.NewServeMux()
	mux.HandleFunc("/message.Greeter/", proxyHandle)
	mux.HandleFunc("/message.Echo/", proxyHandle)
