...
func init() {
	...
	flag.StringVar(&address, "address", ":50050", "proxy address")
...
func echo(conn *grpc.ClientConn, args []string) error {
	...
	r, err := c.Replay(ctx, &message.EchoRequest{Message: msg})
	if err != nil {
		return err
	}
...
func hello(conn *grpc.ClientConn, args []string) error {
	...
	r, err := c.SayHello(ctx, &message.HelloRequest{Name: name})
	if err != nil {
		return err
	}
...
func lots(conn *grpc.ClientConn, args []string) error {
	...
	r, err := c.LotsOfReplies(ctx, &message.HelloRequest{Name: name})
	if err != nil {
		return err
	}
...
func many(conn *grpc.ClientConn, args []string) error {
	...
	go func() {
		...
		for _, name := range args {
			stream.Send(&message.HelloRequest{Name: name})
		}
		stream.CloseSend()
	}()
...
func status(conn *grpc.ClientConn, args []string) error {
	c := message.NewHealthClient(conn)

	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()

	r, err := c.Status(ctx, &message.StatusRequest{})
	if err != nil {
		return err
	}
